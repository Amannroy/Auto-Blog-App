...
const AddBlog = () => {
 ...
 const generateContent = async() => {
  ...
  /**
 * Handles the blog submission form.
 * - Prevents page reload on form submit.
 * - Sets loading state to show the blog is being added.
 * - Collects blog data (title, subtitle, description, category, etc.) and image.
 * - Sends the data to the backend using a POST request via Axios.
 * - On success:
 *    - Shows a success message.
 *    - Clears the form fields.
 * - On failure:
 *    - Shows an error message.
 * - Catches and displays any unexpected errors.
 */
        ...
        <label htmlFor="image">
          ...
          <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
            <div ref={editorRef}></div>
               <button type="button" onChange={generateContent} className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer'>Generate with AI</button>
          </div>
          ...
          <div className='flex gap-2 mt-4'>
            <p>Publish Now</p>
            <input type="checkbox" onChange={e => setIsPublished(e.target.checked)} check={isPublished} className='scale-125 cursor-pointer'/>
          </div>
